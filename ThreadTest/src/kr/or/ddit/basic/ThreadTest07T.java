...
public class ThreadTest07T {
	public static boolean inputCheck; // 자동으로 false로 초기화됨
	...
	public static void main(String[] args) {
		...
		timer.start();  // 카운트 다운 시작...
		
		do {
		user = JOptionPane.showInputDialog("가위 바위 보를 입력하세요.");
//		}while(user==null || !(user.equals("가위") || user.equals("바위") || user.equals("보")) ); - 아래 식과 동일
...
class GameTimer extends Thread{
	...
	public void run() {
		...
		for(int i=5; i>=1; i--) {
			...
			if(ThreadTest07T.inputCheck==true) {  // 입력 완료 여부 검사
				return;
			}
