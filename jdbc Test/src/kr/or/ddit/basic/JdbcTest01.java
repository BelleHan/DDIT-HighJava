...
/*
 	...
 	3. 질의 ==> SQL문장을 DB서버로 보내서 결과를 얻어온다.
 		(Statement객체나 PreparedStatement객체를 이용하여 작업한다.)"HHH 
	...
	public static void main(String[] args) {
		...
		try {
			...
			conn = DriverManager.getConnection(
					"jdbc:oracle:thin:@localhost:1521:xe",  //localhost는 오라클이 설치된 컴퓨터의 IP주소 자리
					"HHS96", "java");
			...
			//				다음번째 레코드 위치로 이동 시키고 그 곳에 데이터가 있으면
			//				true를 반환하고 없으면 false를 반환한다.
			while(rs.next()) {  
				// 포인터가 가리키는 곳의 데이터를 가져오기
				// 형식1) rs.get자료형이름("컬럼명");
				// 형식2) rs.get자료형이름(컬럼번호); ==> 컬럼번호는 1번부터 시작한다. (db는 0번부터 처리하지 않고 1번부터 처리함)
